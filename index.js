...
function buildArgs (source, givenOutput, loop, initialVolume, showOsd, addArgs) {
	let output = '';
...
function Omx (source, output, loop, initialVolume, showOsd) {
	...
	function spawnPlayer (src, out, loop, initialVolume, showOsd, addArgs) {
		let args = buildArgs(src, out, loop, initialVolume, showOsd, addArgs);
	...
	omxplayer.newSource = (src, out, loop, initialVolume, showOsd, addArgs) => {

		if (open) {

			player.on('close', () => { player = spawnPlayer(src, out, loop, initialVolume, showOsd, addArgs); });
			player.removeListener('close', updateStatus);
			writeStdin('q');

		} else {

			player = spawnPlayer(src, out, loop, initialVolume, showOsd, addArgs);
